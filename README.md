...
An asset-pricing regression model explaining a stock's daily returns using the
...
- **Factor returns** (Mkt-RF, SMB, HML, RF): pulled directly from Kenneth French's
  publicly available data library at Dartmouth.
...
3. Interpret the output - Results below.
...
- Single-stock regression on daily data - factor models are more commonly applied
  to diversified portfolios, where stock specific noise averages
...
- Only covers the original 3 factors.
- Uses daily rather than monthly data, monthly data is more standard.
